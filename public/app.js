...
const CATEGORIES_SAIDA = ['Moradia','Alimentação','Transporte','Saúde','Educação','Lazer','Assinaturas','Cartão de Crédito','Dízimo','Outros'];
const CATEGORIES_ENTRADA = ['Salário','Freelance/Aulas','Rendimento','Presente','Outros'];
...
function DEFAULT_DATA(){
  return {settings:null, transactions:[], installments:[], cards:[], cardBills:{}, caixinhas:[], titheStatus:{}, reminders:[], budgets:[]};
}
...
const SPENDING_COLORS = ['#B8863C','#3F7D63','#A6432F','#4A6FA5','#8C6428','#6B5B95','#C97B4A'];
function renderSpendingMap(monthTx){
  ...
  const byCat = {};
  saidas.forEach(t=>{ byCat[t.category] = (byCat[t.category]||0) + Number(t.amount); });
    ...
    <section>
      ...
      <div class="row-list">
        ${DATA.budgets.length===0 ? '<div class="empty"><span class="empty-title">Nenhum teto definido ainda</span>Escolha uma categoria que costuma te surpreender e defina um limite pra ela.</div>' :
          DATA.budgets.slice(0,4).map(b=>renderBudgetRow(b, monthTx)).join('')}
      </div>
    </section>
    <section>
      <div class="section-head">
...
function addTx(){
  ...
  DATA.transactions.push({id:uid(), type:val('tx-type'), personId:val('tx-person'), category:val('tx-category'), description:desc, amount, date:dateStr});
  closeModal();
  persist();
}
...
function renderBudgetRow(b, monthTx){
  ...
  const fillColor = over ? 'var(--garnet)' : (pct>=80 ? 'var(--brass)' : 'var(--verdigris)');
  return `
    <div class="card" style="padding:14px 16px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px; flex-wrap:wrap; gap:6px;">
        ...
        <button class="icon-btn" onclick="removeBudget('${b.id}')">✕</button>
      </div>
...
function renderOrcamentos(mKey){
  ...
  const totalSpent = DATA.budgets.reduce((s,b)=>s + monthTx.filter(t=>t.type==='saida'&&t.category===b.category).reduce((s2,t)=>s2+Number(t.amount),0), 0);
  return `
    ...
    <div class="row-list">
      ${DATA.budgets.length===0 ? '<div class="empty"><span class="empty-title">Nenhum teto ainda</span>Comece com uma categoria só — "Alimentação" costuma ser um bom primeiro teste.</div>' :
...
function openBudgetForm(){
  ...
  openModal(`
    ...
    <div class="modal-actions">
      <button class="btn secondary" onclick="closeModal()">Cancelar</button>
      <button class="btn" onclick="addBudget()">Salvar</button>
    </div>
  `);
}
function addBudget(){
  ...
  if(!category){ if(errEl) errEl.textContent='Escolha uma categoria.'; return; }
...
function addInstallment(){
  ...
  DATA.installments.push({id:uid(), description:desc, personId: personEl?personEl.value:'p1', totalAmount:total, count, paid:0, monthlyAmount, firstDueDate:val('inst-date'), category:val('inst-category')});
  closeModal(); persist();
}
